...
void placerJoueursSurPlateau(Joueur joueurs[], int nbJoueurs, char plateau[TAILLE_PLATEAU][TAILLE_PLATEAU]) {
    ...
    if (nbJoueurs == 2) {
        ...
        printf("Debogage : Joueur 1 (%c) place en (%d, %d).\n", joueurs[0].symbole, joueurs[0].x, joueurs[0].y);
        printf("Debogage : Joueur 2 (%c) place en (%d, %d).\n", joueurs[1].symbole, joueurs[1].x, joueurs[1].y);

    } else if (nbJoueurs == 4) {
        // Joueur 1 commence en bas gauche
        joueurs[0].x = TAILLE_PLATEAU - 1;
        joueurs[0].y = TAILLE_PLATEAU / 4;
        joueurs[0].directionCible = 'H'; // Objectif : haut
        joueurs[0].ligneCible = 0;

        // Joueur 2 commence en haut gauche
        joueurs[1].x = 0;
        joueurs[1].y = TAILLE_PLATEAU / 4;
        joueurs[1].directionCible = 'B'; // Objectif : bas
        joueurs[1].ligneCible = TAILLE_PLATEAU - 1;

        // Joueur 3 commence en bas droite
        joueurs[2].x = TAILLE_PLATEAU - 1;
        joueurs[2].y = 3 * (TAILLE_PLATEAU / 4);
        joueurs[2].directionCible = 'H'; // Objectif : haut
        joueurs[2].ligneCible = 0;

        // Joueur 4 commence en haut droite
        joueurs[3].x = 0;
        joueurs[3].y = 3 * (TAILLE_PLATEAU / 4);
        joueurs[3].directionCible = 'B'; // Objectif : bas
        joueurs[3].ligneCible = TAILLE_PLATEAU - 1;

        for (int i = 0; i < nbJoueurs; i++) {
            plateau[joueurs[i].x][joueurs[i].y] = joueurs[i].symbole;
            printf("Debogage : Joueur %d (%c) placé en (%d, %d).\n", i + 1, joueurs[i].symbole, joueurs[i].x, joueurs[i].y);
        }
    } else {
